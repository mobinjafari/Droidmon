...
import ana.ndk.droidmon.utils.Files;
import ana.ndk.droidmon.utils.Logger;
import ana.ndk.droidmon.utils.MethodApiType;
...
public class InstrumentationManager implements IXposedHookLoadPackage, IXposedHookZygoteInit {
	...
	public void instrumentApp() throws FileNotFoundException, IOException
	{
		...
		for (HookConfig hookConfig : instrumentationConfiguration.hookConfigs) {
			hook(new ana.ndk.droidmon.MethodHookImpl(hookConfig.class_name,hookConfig.method,hookConfig.thisObject,hookConfig.type));
		}	}
	...
	private static void hook(ana.ndk.droidmon.MethodHookImpl methodHook) {
		hook(methodHook, null);
	}

	private static void hook(final ana.ndk.droidmon.MethodHookImpl methodHook, ClassLoader classLoader) {
		try {
